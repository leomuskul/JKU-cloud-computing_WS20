...
<p>It is possible to build an own server wich would compute machine learning tasks like Google Colaboratory does. But what is the losses of time that are appears with remote connection? How better is performance? For that the next goals would be:</p> 
